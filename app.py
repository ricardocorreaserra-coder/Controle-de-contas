...
# ── Google Sheets ──────────────────────────────────────────────────────────────
SCOPES = [
    httpswww.googleapis.comauthspreadsheets,
    httpswww.googleapis.comauthdrive,
]
...
# HEADER
# ══════════════════════════════════════════════════════════════════════════════
st.markdown('div class=main-headerspan style=font-size1.6rem💰span'
            'span style=font-size1.3rem;font-weight700Controle de Contasspandiv',
            unsafe_allow_html=True)
    ...
    if df_d.empty
        ...
        if not df_filtrado.empty
            ...
            if st.button(🗑 Excluir despesa, type=secondary, key=btn_excluir_d)
                ...
                st.success(fDespesa #{id_excluir} excluída!)
                st.rerun()
        else
    ...
    if df_p.empty
        ...
        if not df_p2.empty
            ...
            if ba2.button(✔ Dar baixa, type=primary)
                ...
                st.success(fParcela #{pid_acao} marcada como paga!)
                st.rerun()
            if ba3.button(↩ Estornar)
                atualizar_parcela(int(pid_acao), pendente)
                st.warning(fParcela #{pid_acao} estornada para pendente.)
                st.rerun()
...
with tab_cc_rec
    ...
    if st.button(🗑 Excluir receita, type=secondary)
        ...
        st.success(fReceita #{rid_excluir} excluída!)
        st.rerun()